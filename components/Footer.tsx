      ...
      <Container>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-12 lg:gap-16 mb-20">
          <div className="col-span-2 lg:col-span-1">
            <h3 className="font-label text-white text-xl mb-6 font-bold tracking-tighter">FAB <span className="text-primary italic">CLINIC</span></h3>
          ...
          {[
            ...
            <nav key={col.title} aria-label={col.title}>
              <h4 className="font-label text-primary text-[11px] tracking-[0.3em] uppercase mb-8 font-bold">
                {col.title}
              </h4>
        ...
        {/* Disclaimer */}
        <p className="mb-16 text-center max-w-4xl mx-auto font-sans text-[10px] md:text-[11px] italic text-background-light/30 leading-relaxed uppercase tracking-[0.25em] font-bold">
          {t('disclaimer')}
        </p>

        {/* Bottom Bar */}
        <div className="flex flex-col md:flex-row justify-between items-center gap-8 text-background-light/40 uppercase tracking-[0.25em] font-bold">
          <p className="font-sans text-[10px]">{t('copyright')}</p>

          {/* Payment Badges */}
          <div className="flex flex-wrap justify-center gap-4">
            {['iDEAL', 'Amex', 'Visa', 'Mastercard', 'STP'].map((method) => (
              <span key={method} className="font-label text-[10px] tracking-widest uppercase border border-background-light/5 px-4 py-1.5 rounded-pill hover:border-primary/20 transition-colors duration-200 cursor-default">
                {method}
              </span>
            ))}
          </div>
